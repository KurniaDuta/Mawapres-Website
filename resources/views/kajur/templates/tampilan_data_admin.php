<main class="bg-[#F7F7F8] flex-1 m-8 rounded-xl p-8 flex flex-col gap-8 ml-[340px] min-h-[560px]">
    <section class="flex w-full justify-between">
        <!-- Tombol untuk menambah admin yang sekarang menjadi link -->
        <a href="submisi_admin.php" class="bg-[#2862C6] flex justify-center text-white px-6 py-3 rounded-sm gap-3">
            ...
            <p class="font-medium">Tambah Admin</p>
        </a>
        <!-- search bar -->
        ...
        <table class="w-full border-separate border-spacing-1">
            <thead>
                <tr class="font-semibold">
                    ...
                    <th class="bg-gray-200 rounded p-3">Aksi</th>
                </tr>
            </thead>
...
<script>
// Fungsi untuk menambah baris admin baru
function addAdmin(nip, name) {
    // Membuat baris baru untuk admin
    const tableBody = document.getElementById('adminTableBody');
    const row = document.createElement('tr');
    row.classList.add('bg-white', 'cursor-pointer', 'hover:bg-gray-100');

    // Menambahkan kolom untuk NIP
    const nipCell = document.createElement('td');
    nipCell.classList.add('p-3', 'text-center');
    nipCell.textContent = nip;
    row.appendChild(nipCell);

    // Menambahkan kolom untuk nama dosen
    const nameCell = document.createElement('td');
    nameCell.classList.add('p-3', 'text-center');
...
// Fungsi untuk menghapus admin berdasarkan NIP
function deleteAdmin(nip) {
    if (confirm('Are you sure you want to delete this admin?')) {
        const rows = document.querySelectorAll('#adminTableBody tr');
        rows.forEach(row => {
            if (row.cells[0].textContent === nip.toString()) {
                row.remove();
            }
        });
        console.log('Admin with NIP ' + nip + ' deleted.');
    }
}

// Contoh menambah admin baru secara otomatis
addAdmin(123456789, 'John Doe');
addAdmin(987654321, 'Jane Smith');
</script>
